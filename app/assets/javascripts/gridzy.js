...
(function () {
	...
	var defaultOptionsTpl = {
		...
		onBeforeOptionsChanged: function () {}, // deprecated since v2.4.0
		onOptionsChanged: function () {}, // deprecated since v2.4.0
		onBeforeRender: function () {}, // deprecated since v2.4.0
		onRender: function () {}, // deprecated since v2.4.0
	...
	var helper = {
		...
		processLazyLoading: function () {
			...
			while (pos--) {
				...
				if (rect.width > 0 && rect.height > 0 && xPos > 1 - edge && xPos < edge && yPos > 1 - edge && yPos < edge) {
			...
			for (pos = 0; pos < imagesToLoad.length; pos++) {
				...
				helper.dispatchEvent(imagesToLoad[pos].item.contentElement, 'ItemLoading', {instance: this});
		...
		dispatchEvent: function (element, eventName, detailObject) {
			element.dispatchEvent(new CustomEvent('gridzy' + eventName, {
				bubbles: true,
				detail: detailObject || {},
			}));
		}
	...
	window.Gridzy = function (element, options) {
		...
		if (isNewInstance) {
			...
			// trigger gridzyBeforeInit event
			helper.dispatchEvent(element, 'BeforeInit', {instance: self, optionsToSet: options});
			...
			helper.registerReadOnlyObjectParameters(self, {
				...
				_imageMutationObserverConfig: {
					attributes: false,
					attributeFilter: ["src"],
					subtree: true
				},
				_imageMutationObserver: new MutationObserver(function(mutations) {
					if (mutationObservation) {
						mutations.forEach(function(mutation) {
							switch (mutation.type) {
								case 'attributes':
									if (mutation.target.nodeName !== 'IMG') return;
									var img = mutation.target;
									var item = img;
									while (item && typeof item.gridzyItem === 'undefined') {
										item = item.parentNode;
									}
									if (item && item.gridzyItem) {
										var gridzyInstance = item.gridzyItem.gridzyInstance;
										item.gridzyItem.destroy();
										gridzyInstance.syncChild(item);
									}
							}
						});
					}
				}),
				_renderOnResizeObserverStatus: {enabled:false},
				_renderOnResizeObserver: (function() {
					var prevWidth = null;
					return new ResizeObserver(function(entries) {
						var entry = entries[0];
						var width = entry.contentBoxSize?.[0]?.inlineSize || entry.contentRect?.width;
						if (width !== prevWidth) {
							prevWidth = width;
							entry.target?.gridzy?.render();
						}
					});
				})(),
			});
			...
			// trigger gridzyInit event
			helper.dispatchEvent(self.element, 'Init', {instance: self});
		} else {
			self.setOptions(options);
		}
	...
	Gridzy.prototype = {
		...
		render: function () {
			...
			this._options.onBeforeRender(this); // callback functions are deprecated since v2.4.0
			helper.dispatchEvent(this.element, 'BeforeRender', {instance: this});
			...
			// execute user defined call back function
			this._options.onRender(this); // callback functions are deprecated since v2.4.0
			helper.dispatchEvent(this.element, 'Render', {instance: this});
		},
		...
		_resetImageMutationObserver: function () {
			var config = this._imageMutationObserverConfig,
				changed = false,
				observed = config.attributes,
				observe;

			if (config.attributes !== this._options.autoSyncImageMutation) {
				config.attributes = !config.attributes;
				changed = true;
			}
			observe = config.attributes;
			if (changed) {
				...
				if (observe) {
					this._imageMutationObserver.observe(this.element, helper.objectAssign({}, config));
				}
			}
		},
		_resetRenderOnResizeObserver: function () {
			var status = this._renderOnResizeObserverStatus;
			if (status.enabled !== this._options.autoRenderOnResize) {
				if (this._options.autoRenderOnResize) {
					this._renderOnResizeObserver.observe(this.element, {box: 'content-box'});
					status.enabled = true;
				}
				else {
					this._renderOnResizeObserver.unobserve(this.element);
		...
		_createItem: function (contentElement) {
			...
			helper.dispatchEvent(contentElement, 'ItemBeforeInit', {instance: this});
			...
			helper.changeMultipleClassNames(item.contentElement, true, ['gridzyItem', 'gridzyItemInitializing']);
				...
				(function (e) {
					...
					if (!src && (lazySrc || lazySrcset)) {
						...
						if (!attributeWidth || !attributeHeight) {
							...
							e.src = helper.getTransparentPixelSrc();
						}
					} else {
						loading = true;
					}
			...
			if (loading === true) {
				item.contentElement.classList.add('gridzyItemLoading');
				helper.dispatchEvent(item.contentElement, 'ItemLoading', {instance: this});
			}

			item.images.forEach(function (image, index) {
				var onLoad = function () {
					if (image.src === helper.getTransparentPixelSrc()) {
						return;
					}
			...
			helper.dispatchEvent(item.contentElement, 'ItemInit', {instance: this});
		...
		_processInsertQueue: function (maxQuietTimeReached) {
			...
			if (maxQuietTimeReached || maxQueueTimeReached) {
				...
				while (queueItemsPos--) {
					...
					helper.dispatchEvent(queueItem.item.contentElement, 'ItemReady', {instance: this});
				}
			...
			if (lazyLoadImagesQueue === null && images.length) {
				...
				if ('IntersectionObserver' in window) {
					var intersectionObserver = new IntersectionObserver(function(entries) {
						...
						while (pos--) {
							if (entries[pos].intersectionRatio > 0) {
								helper.processLazyLoading();
								return;
							}
						}
					...
					images.forEach(function(image) {
						intersectionObserver.observe(image.image);
					});
				}
			}
		...
		_onImageFinished: function (item) {
			...
			if (!item.imagesFinished && !helper.inArray(item.imagesStatus, this.IMG_LOADING) && !helper.inArray(item.imagesStatus, this.IMG_LOADING_READY) && !helper.inArray(item.imagesStatus, this.IMG_READY)) {

				// prevent multiple executions
				item.imagesFinished = true;
				...
				helper.dispatchEvent(item.contentElement, 'ItemComplete', {instance: this, status: item.imagesStatus[0]});
			}
		},
		...
		_updateOptionParameters: function (options) {
			...
			this._options.onBeforeOptionsChanged(this); // callback functions are deprecated since v2.4.0
			helper.dispatchEvent(this.element, 'BeforeOptionsChanged', {instance: this});
			...
			// set event handlers
			if (typeof options.onBeforeOptionsChanged === 'function') this._options.onBeforeOptionsChanged = options.onBeforeOptionsChanged; // deprecated since v2.4.0
			if (typeof options.onOptionsChanged === 'function') this._options.onOptionsChanged = options.onOptionsChanged; // deprecated since v2.4.0
			if (typeof options.onBeforeRender === 'function') this._options.onBeforeRender = options.onBeforeRender; // deprecated since v2.4.0
			if (typeof options.onRender === 'function') this._options.onRender = options.onRender; // deprecated since v2.4.0
			...
			// set autoSyncImageMutation
			if (typeof options.autoSyncImageMutation !== 'undefined' && options.autoSyncImageMutation !== null) this._options.autoSyncImageMutation = helper.isTruly(options.autoSyncImageMutation);

			// set autoRenderOnResize
			if (typeof options.autoRenderOnResize !== 'undefined' && options.autoRenderOnResize !== null) this._options.autoRenderOnResize = helper.isTruly(options.autoRenderOnResize);
			...
			// reset image mutation observer to new options
			this._resetImageMutationObserver();

			// reset render on resize observer to new options
			this._resetRenderOnResizeObserver();

			this._options.onOptionsChanged(this); // callback functions are deprecated since v2.4.0
			helper.dispatchEvent(this.element, 'OptionsChanged', {instance: this});
		}
	};
